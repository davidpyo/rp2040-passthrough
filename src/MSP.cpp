...
uint8_t MSP_Check(uint8_t MSP_buf[], uint8_t buf_size) {
	...
	if (MSP_type == MSP_API_VERSION && MSP_crc == 0x01) { // MSP_API_VERSION
		...
		MSP_buf[6] = 'T'; // BETAFLIGHT_IDENTIFIER "T"
		MSP_buf[7] = 'F'; // BETAFLIGHT_IDENTIFIER "F"
		MSP_buf[8] = 'L'; // BETAFLIGHT_IDENTIFIER "L"
		MSP_OSize = 9;
	}
	...
	else if (MSP_type == MSP_BATTERY_STATE) { // MSP_BATTERY_STATE (130)
		MSP_buf[2] = 0x3E;
		MSP_buf[3] = 0x09;                 // payload size
		MSP_buf[4] = MSP_BATTERY_STATE;
		MSP_buf[5] = 0x00;
		MSP_buf[6] = 0x00;
		MSP_buf[7] = 0x00;
		MSP_buf[8] = 0x00;
